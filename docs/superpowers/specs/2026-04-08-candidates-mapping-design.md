...
**日期：** 2026-04-08  
**範圍：** 2012 年以後，四大選舉類型（國家元首、縣市首長、立委、縣市議員）

---

## 目標

產生並維護一份 `candidates.yaml`，作為候選人黨派歸屬的 mapping 檔，供以下用途對照：

- 政治獻金申報資料的黨派歸屬劃分
- 當選人/落選人派任政府職務或機構的身份對照
...
- name: string        # 完整原始姓名（含英文、原住民族名）
  ...
  birthday: string|integer|null  # 見生日格式說明
  elections:
    - year: integer   # 西元年份
...
| 來源 | 原始格式 | 記錄方式 |
|------|---------|---------|
| president / mayor | `出生年次`（西元年，僅年份） | 整數 `1959` |
| legislator / council | 民國 `e/mm/dd`（如 `058/11/26`） | 字串 `1969/11/26`（轉西元） |
| 無資料 | — | `null` |
...
type enum 清單，每個 type 帶 `aliases` 陣列，記錄對應的原始文字寫法（CSV 檔名前綴 + 選舉名稱的組合模式）。
...
政治獻金 CSV 欄位：`擬參選人`、`參選公職名稱`

對照流程：

```
1. 載入 candidates.yaml → 建立 in-memory index
   key: normalize(姓名) → list of election records

2. 每筆獻金：
   a. normalize(擬參選人) → 查 index → O(1)
   b. parse year + resolve type（查 election_types aliases）
   c. 在候選人的 elections 找 year + type → 取得 party
```

### 同名處理

若政治獻金同年同選舉出現兩個完全相同姓名，`candidates.yaml` 中兩人各有不同 id，需人工比對後以 id 區分。
...
- **不含：** 鄉鎮區長、里長、鄉鎮市民代表（資料量龐大，超出現階段範圍）
