...
def generate_id(
    name: str,
    birthyear: Union[str, int, None],
    year: Union[int, str, None] = None,
    age: Union[int, str, None] = None,
) -> str:
    """
    產生候選人唯一 ID
    birthyear 可以接受 yyyy-mm-dd 或 yyyy 或 None
    year 為 int, 當 birthyear 為 None 時使用, 格式為 yyyy
    age 為 int, 此為 birthyear 為 None 時使用, 格式 xx
    ID 格式：id_{正規化姓名} 或 id_{正規化姓名}_{yyyy}
    """
    base = normalize_name(name)

    if birthyear:
        birth_year = str(birthyear)[:4]
        ...
        raise ValueError(f"❌ {name} 組 id 失敗. birthyear={birthyear} & year={year} & age={age}")
